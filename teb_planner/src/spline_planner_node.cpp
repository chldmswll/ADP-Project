#include "teb_planner/spline_planner_node.hpp"
...
void ObstacleSpliner::spliner_loop() {
  ...
  RCLCPP_DEBUG(this->get_logger(), "Received %zu obstacles", obs_.obstacles.size());
  if (!obs_.obstacles.empty()) {
    ...
    auto spline_duration = std::chrono::duration_cast<std::chrono::milliseconds>(spline_end - spline_start);
    
    wpnts = result.first;
    mrks = result.second;
    
    RCLCPP_INFO(this->get_logger(), 
                "do_spline completed in %ld ms, generated %zu waypoints", 
                spline_duration.count(), wpnts.wpnts.size());
    
    // If splining failed and returned empty waypoints, provide fallback global trajectory
    if (wpnts.wpnts.empty() && !gb_scaled_wpnts_->wpnts.empty()) {
      RCLCPP_WARN(this->get_logger(), 
                  "Splining failed or returned empty waypoints. Using fallback global trajectory.");
      double wpnt_dist = gb_scaled_wpnts_->wpnts[1].s_m - gb_scaled_wpnts_->wpnts[0].s_m;
      int start_idx = static_cast<int>(cur_s_ / wpnt_dist) % gb_max_idx_;
      for (int i = 0; i < 20 && (start_idx + i) % gb_max_idx_ < static_cast<int>(gb_scaled_wpnts_->wpnts.size()); ++i) {
        int idx = (start_idx + i) % gb_max_idx_;
        const auto& gb_wpnt = gb_scaled_wpnts_->wpnts[idx];
        f110_msgs::msg::Wpnt wpnt;
        wpnt.id = i;
        wpnt.x_m = gb_wpnt.x_m;
        wpnt.y_m = gb_wpnt.y_m;
        wpnt.s_m = gb_wpnt.s_m;
        wpnt.d_m = gb_wpnt.d_m;
        wpnt.vx_mps = gb_wpnt.vx_mps * 0.8;  // 약간 감속
        wpnts.wpnts.push_back(wpnt);
      }
      wpnts.header.stamp = this->now();
      wpnts.header.frame_id = "map";
      wpnts.ot_side = "none";
      wpnts.side_switch = false;
      
      // Fallback 경로에 대한 마커도 추가
  ...
  // Ensure all markers have proper header and action
  for (auto& marker : mrks.markers) {
    marker.header.stamp = this->now();
    marker.header.frame_id = "map";
    if (marker.action == 0) {  // If not set, set to ADD
  ...
  RCLCPP_DEBUG(this->get_logger(), "Published %zu waypoints and %zu markers", 
  ...
  const std::vector<f110_msgs::msg::Wpnt>& gb_wpnts)
{
  ...
  RCLCPP_INFO(this->get_logger(), "Filtered obstacles: %zu -> %zu close obstacles", 
               obstacles.obstacles.size(), close_obs.size());
  
  if (!close_obs.empty()) {
    RCLCPP_INFO(this->get_logger(), "Processing %zu close obstacles, starting TEB path generation", close_obs.size());
    ...
    if (!check_ot_side_possible(ot_side)) {
      RCLCPP_WARN(this->get_logger(), "Overtaking side '%s' not possible, setting danger_flag", ot_side.c_str());
      danger_flag = true;
    }
    
    if (!danger_flag && converter_) {
      auto teb_start = std::chrono::high_resolution_clock::now();
      
      // 1. 초기 경로 생성 (전역 경로에서 일부 가져오기)
      auto init_path_start = std::chrono::high_resolution_clock::now();
      std::vector<Eigen::Vector2d> initial_path = get_initial_path_from_global(closest_obs);
      auto init_path_end = std::chrono::high_resolution_clock::now();
      auto init_path_duration = std::chrono::duration_cast<std::chrono::milliseconds>(init_path_end - init_path_start);
      RCLCPP_INFO(this->get_logger(), "Initial path generation: %ld ms, size: %zu", 
                  init_path_duration.count(), initial_path.size());
      ...
      if (!initial_path.empty()) {
        ...
        auto metrics_duration = std::chrono::duration_cast<std::chrono::milliseconds>(metrics_end - metrics_start);
        
        RCLCPP_INFO(this->get_logger(), 
                    "Metrics calculation: %ld ms - Narrowness: %.3f, Turning Complexity: %.3f", 
                    metrics_duration.count(), narrowness, turning_complexity);
        ...
        RCLCPP_INFO(this->get_logger(), "TEB optimization: %ld ms", optimize_duration.count());
        
        // 9. 최적화된 경로를 waypoint로 변환
        auto path_with_velocities = teb.get_path_with_velocities();
        
        RCLCPP_INFO(this->get_logger(), "TEB generated path with %zu points", path_with_velocities.size());
        
        if (path_with_velocities.empty()) {
          RCLCPP_WARN(this->get_logger(), "TEB path is empty! Falling back to global trajectory");
          // Fallback: 글로벌 경로의 일부를 그대로 사용
          int start_idx = static_cast<int>(cur_s_ / wpnt_dist) % gb_max_idx_;
          for (int i = 0; i < 20 && (start_idx + i) % gb_max_idx_ < static_cast<int>(gb_wpnts.size()); ++i) {
            int idx = (start_idx + i) % gb_max_idx_;
            const auto& gb_wpnt = gb_wpnts[idx];
            f110_msgs::msg::Wpnt wpnt;
            wpnt.id = i;
            wpnt.x_m = gb_wpnt.x_m;
            wpnt.y_m = gb_wpnt.y_m;
            ...
            // Fallback 경로에 대한 마커도 생성
            auto mrk = xyv_to_markers(gb_wpnt.x_m, gb_wpnt.y_m, gb_wpnt.vx_mps * 0.8, mrks.markers.size());
        ...
        for (size_t i = 0; i < path_with_velocities.size(); i++) {
          ...
          // d 값이 비정상적으로 크면 (Frenet 변환 오류 가능) 스킵
          // 일반적으로 트랙 폭은 3-5m 이내이므로, 10m 이상이면 비정상
          if (std::abs(d) > 10.0) {
            RCLCPP_DEBUG(this->get_logger(), 
                        "Skipping point %zu: abnormal d value (d=%.3f), likely Frenet conversion error", 
                        i, d);
            continue;  // 이 포인트 스킵하고 다음으로
          }
          ...
          if (gb_wpnt_i < static_cast<int>(gb_wpnts.size())) {
            if (std::abs(d) > 0.1) {  // spline_resolution
              ...
              // Track bounds 체크: d가 tb_dist보다 크면 경계를 벗어남
              // 하지만 약간의 여유를 두고 체크 (spline_bound_mindist_)
              if (std::abs(d) > std::abs(tb_dist) - spline_bound_mindist_) {
                RCLCPP_WARN(this->get_logger(),
                  "Evasion trajectory too close to TRACKBOUNDS (d=%.3f, tb_dist=%.3f), aborting evasion", 
                  d, tb_dist);
                danger_flag = true;
                break;
              }
        ...
        auto teb_end = std::chrono::high_resolution_clock::now();
        auto teb_duration = std::chrono::duration_cast<std::chrono::milliseconds>(teb_end - teb_start);
        
        RCLCPP_INFO(this->get_logger(), 
                    "TEB total: %ld ms (Frenet conversion: %ld ms), Generated %zu waypoints and %zu markers", 
                    teb_duration.count(), conversion_duration.count(), wpnts.wpnts.size(), mrks.markers.size());
        
        // LINE_STRIP 마커 추가 (연두색 선으로 표시) - waypoint가 있을 때만
        // 원래 Python 코드에는 없지만, 시각화를 위해 추가
        if (!wpnts.wpnts.empty()) {
          visualization_msgs::msg::Marker line_marker;
          line_marker.header.frame_id = "map";
          line_marker.header.stamp = this->now();
          ...
          line_marker.color.a = 1.0;
          line_marker.color.r = 0.75;  // 원래 Python CYLINDER 색상과 유사
          line_marker.color.g = 0.75;  // 연두색
          line_marker.color.b = 0.75;
          line_marker.lifetime = rclcpp::Duration(0, 0);  // 무한 lifetime
          ...
          if (line_marker.points.size() >= 2) {  // 최소 2개 포인트 필요
            mrks.markers.push_back(line_marker);
            RCLCPP_INFO(this->get_logger(), "Added LINE_STRIP marker with %zu points for evasion path", line_marker.points.size());
          }
        }
      } else {
      ...
      if (danger_flag) {
        RCLCPP_WARN(this->get_logger(), "Danger flag set, skipping trajectory generation");
      }
      if (!converter_) {
        RCLCPP_WARN(this->get_logger(), "Frenet converter not initialized");
      }
      RCLCPP_INFO(this->get_logger(), "TEB path generation skipped: danger_flag=%s, converter_=%s", 
                  danger_flag ? "true" : "false", converter_ ? "initialized" : "null");
    }
    ...
    RCLCPP_INFO(this->get_logger(), 
                "do_spline result: danger_flag=%s, wpnts.size()=%zu, close_obs.size()=%zu", 
                danger_flag ? "true" : "false", wpnts.wpnts.size(), close_obs.size());
    
    if (!danger_flag && !wpnts.wpnts.empty()) {
      ...
      if (danger_flag) {
        RCLCPP_WARN(this->get_logger(), "Danger flag set - evasion not safe. Using fallback global trajectory.");
        // Fallback: 글로벌 경로를 그대로 사용 (약간 감속)
        int start_idx = static_cast<int>(cur_s_ / wpnt_dist) % gb_max_idx_;
        for (int i = 0; i < 20 && (start_idx + i) % gb_max_idx_ < static_cast<int>(gb_wpnts.size()); ++i) {
          int idx = (start_idx + i) % gb_max_idx_;
          const auto& gb_wpnt = gb_wpnts[idx];
          ...
          wpnt.s_m = gb_wpnt.s_m;
          wpnt.d_m = gb_wpnt.d_m;
          wpnt.vx_mps = gb_wpnt.vx_mps * 0.7;  // 감속하여 안전하게
          wpnts.wpnts.push_back(wpnt);
        }
        if (!wpnts.wpnts.empty()) {
          wpnts.ot_side = ot_side;
      ...
      } else if (wpnts.wpnts.empty()) {
        RCLCPP_WARN(this->get_logger(), 
                    "Generated waypoints are empty. Clearing waypoints. "
                    "danger_flag=%s, close_obs.size()=%zu", 
                    danger_flag ? "true" : "false", close_obs.size());
        wpnts.wpnts.clear();
        // 마커는 DELETEALL로 정리
        visualization_msgs::msg::Marker del_mrk;
        del_mrk.header.stamp = this->now();
  ...
  double x, double y, double v, size_t id)
{
  ...
  if (from_bag_) {
    ...
    mrk.color.g = 0.75;  // 원래 Python 코드와 동일하게 (연두색처럼 보임)
  }
