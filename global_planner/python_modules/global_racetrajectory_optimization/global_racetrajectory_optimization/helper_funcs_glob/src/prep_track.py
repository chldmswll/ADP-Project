...
def prep_track(reftrack_imp: np.ndarray,
    ...
    # Ensure reftrack_imp is properly formatted before calling spline_approximation
    # This is critical as spline_approximation may have issues with certain array formats
    if not isinstance(reftrack_imp, np.ndarray):
        reftrack_imp = np.array(reftrack_imp, dtype=np.float64)
    else:
        reftrack_imp = np.asarray(reftrack_imp, dtype=np.float64)
    
    # Ensure it's a 2D array with 4 columns
    if len(reftrack_imp.shape) != 2:
        raise ValueError(f"reftrack_imp must be 2D array, got shape: {reftrack_imp.shape}")
    if reftrack_imp.shape[1] != 4:
        raise ValueError(f"reftrack_imp must have 4 columns [x, y, w_right, w_left], got {reftrack_imp.shape[1]} columns")
    
    # Ensure contiguous array (required by some numpy operations)
    reftrack_imp = np.ascontiguousarray(reftrack_imp)
    
    # Remove any duplicate consecutive points
    if reftrack_imp.shape[0] > 1:
        diffs = np.diff(reftrack_imp[:, :2], axis=0)
        dists = np.linalg.norm(diffs, axis=1)
        keep_mask = np.concatenate(([True], dists > 0.001))
        if not np.all(keep_mask):
            if debug:
                print(f"WARNING: Removing {np.sum(~keep_mask)} duplicate points from track")
            reftrack_imp = reftrack_imp[keep_mask]
    
    # Ensure all values are finite
    if not np.all(np.isfinite(reftrack_imp)):
        raise ValueError("reftrack_imp contains NaN or Inf values")
    
    if debug:
        print(f"DEBUG prep_track: reftrack_imp shape: {reftrack_imp.shape}, dtype: {reftrack_imp.dtype}, "
              f"is contiguous: {reftrack_imp.flags['C_CONTIGUOUS']}")

    # smoothing and interpolating reference track
    # Note: spline_approximation has known bugs with certain input formats
    # We'll try it first, but fall back to simple interpolation if it fails
    if debug:
        print(f"DEBUG prep_track: Attempting spline_approximation...")
    
    try:
        # Try spline_approximation with full track data (it expects 4 columns)
        reftrack_interp = tph.spline_approximation. \
            spline_approximation(track=reftrack_imp,
                                 ...
                                 debug=debug)
        if debug:
            print(f"DEBUG prep_track: spline_approximation succeeded. Result shape: {reftrack_interp.shape}")
    except Exception as e:
        # If spline_approximation fails, use simple linear interpolation as fallback
        if debug:
            print(f"WARNING: spline_approximation failed: {e}. "
                  f"Using simple linear interpolation as fallback...")
        try:
            from .interp_track import interp_track
            # Use simple linear interpolation with the desired step size
            stepsize = stepsize_opts.get("stepsize_reg", 1.0)
            reftrack_interp = interp_track(reftrack=reftrack_imp, stepsize_approx=stepsize)
            if debug:
                print(f"INFO: Used simple linear interpolation. Result shape: {reftrack_interp.shape}")
        except Exception as e2:
            raise RuntimeError(f"Both spline_approximation and simple interpolation failed. "
                             f"spline_approximation error: {e}. "
                             f"interpolation error: {e2}. "
                             f"reftrack_imp shape: {reftrack_imp.shape}, dtype: {reftrack_imp.dtype}, "
                             f"first few points:\n{reftrack_imp[:5]}") from e2
