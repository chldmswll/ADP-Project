...
file_paths = {"veh_params_file": "/home/misys/forza_ws/race_stack/stack_master/config/global_planner/racecar_f110.ini"}
...
mintime_opts = {"tpadata": None,
                ...
                "use_hybrid_optimization": True,     # Enable MINTIME + MINCURV + CLOTHOID
                "use_multipass": False}             # Enable multi-pass optimization
...
if opt_type == 'mincurv':
    ...
    # Check if hybrid optimization is enabled
    if mintime_opts.get("use_hybrid_optimization", False):
        # ===== HYBRID OPTIMIZATION: MINTIME + MINCURV + CLOTHOID =====
        if debug:
            print("\n" + "="*80)
        ...
        if debug:
            print("[STAGE 1] Running mintime optimization...")
        
        alpha_opt, v_opt, reftrack_interp_tmp, a_interp_tmp, normvec_normalized_interp_tmp = \
        ...
        if reftrack_interp_tmp is not None:
            reftrack_interp = reftrack_interp_tmp
        if normvec_normalized_interp_tmp is not None:
            normvec_normalized_interp = normvec_normalized_interp_tmp
        
        if debug:
            print(f"✓ Mintime complete. Max speed: {np.max(v_opt):.3f} m/s\n")
        
        # STAGE 2: MINCURV REOPTIMIZATION
        if debug:
            print("[STAGE 2] Running mincurv reoptimization for smoothness...")
        ...
        # Set artificial narrow track widths for mincurv
        ...
        # STAGE 3: CLOTHOID SMOOTHING (will be done in later section)
        hybrid_optimization_done = True
    
    else:
        # ===== STANDARD MINTIME OPTIMIZATION =====
        if debug:
            print("\n" + "="*80)
            print("STANDARD MINTIME OPTIMIZATION")
            print("="*80 + "\n")
...
if opt_type == 'mintime' and mintime_opts["reopt_mintime_solution"] and not mintime_opts.get("use_hybrid_optimization", False):

    # Skip if hybrid optimization already performed mincurv reoptimization
...
try:
    ...
    # Use the optimized raceline as reference and recalculate with smaller step size
