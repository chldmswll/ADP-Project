    ...
    double track_length)
{
    ...
    // 속도 조정 (곡률 반영은 외부에서 계산한 curvature_now 사용)
    const double curvature_now_for_speed = Curvature::calc_curvature(idx_nearest_wp_, wp_);
    speed_command_ = speed_adjust_lat_err(speed_command_, lat_e_norm, curvature_now_for_speed);
    ...
    // 스탠리와 곡률은 외부 cpp (stanley.cpp, curvature.cpp)에서 계산해서 받아옴
    const double stanley_angle = Stanley::calc_angle(
        yaw, lat_e_norm, idx_nearest_wp_, wp_, frenet_, speed_now_, logger_warn
    );
    const double curvature_now = Curvature::calc_curvature(idx_nearest_wp_, wp_);
    const double curvature_future = Curvature::estimate_future_curvature_change(idx_nearest_wp_, wp_);
    
    // 순수추종과 스탠리를 곡률 기반으로 결합
    double steering_angle = calc_hybrid_steering(
        L1_point, L1_distance, yaw, lat_e_norm, v,
        stanley_angle, curvature_now, curvature_future
    );
...
double PP_Controller::speed_adjust_lat_err(double global_speed, double lat_e_norm, double curvature_now) const {
    // 곡률 정보는 curvature.cpp에서 계산한 값을 받아옴
    constexpr double curvature_norm_scale = 0.65;
    const double lat_e_coeff = clip(lat_err_coeff, 0.0, 1.0);
    const double lat_e = lat_e_norm * 2.0;
    const double curv = clip(curvature_now / curvature_norm_scale, 0.0, 1.0);
 ...
 * 스탠리 각도와 곡률 정보는 외부에서 계산해서 받아옴 (stanley.cpp, curvature.cpp)
 * 순수추종만으로는 동작하지 않음 (스탠리와 곡률 필수)
 */
double PP_Controller::calc_hybrid_steering(const Vec2& L1_point,
    ...
    double lat_e_norm,
    const Vec2& v,
    double stanley_angle,      // stanley.cpp에서 계산한 값
    double curvature_now,      // curvature.cpp에서 계산한 값
    double curvature_future)   // curvature.cpp에서 계산한 값
{
    // 곡률 정규화 스케일 (0.4 ~ 0.8 사이에서 튜닝 가능, 기본값 0.65)
    constexpr double curvature_norm_scale = 0.45;

    // 순수추종 스티어 (pp.cpp 내부에서만 계산)
    ...
    // linear interpolation: 순수추종과 스탠리 결합
    double hybrid_steer = stanley_weight * stanley_angle + (1.0 - stanley_weight) * pp;
    ...
    const Vec2& v)
{
    ...
    // 곡률 정보는 외부에서 받아와야 하므로, 여기서는 0으로 설정 (또는 파라미터로 받기)
    const double speed_for_lu = speed_adjust_lat_err(speed_la_for_lu, lat_e_norm, 0.0);
